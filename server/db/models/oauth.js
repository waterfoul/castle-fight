...
OAuth.V2 = (accessToken, refreshToken, profile, done) =>
  OAuth.findOrCreate({
    ...
    .spread((oauth) => {
      logger.info(profile);
      logger.info('provider:%s will log in user:{name=%s uid=%s}',
        profile.provider,
  ...
  ({
    ...
    if (undefinedKeys.length) {
      for (let key in config) {
        if (!config[key]) logger.info('provider:%s: needs environment var %s', provider, key);
      }
      logger.info('provider:%s will not initialize', provider);
      return;
    }
